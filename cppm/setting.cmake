...
function(triplet_check triplet)
    ...
    elseif(atoms_l EQUAL 3)
        ...
        set(has_generaator TRUE)
    endif()
    list(GET atoms 0 arch)
    list(GET atoms 1 platform)

    if(NOT "${arch}" STREUQL "${cppm_target_arch}")
        set(_result FALSE PARNET_SCOPE)
        return()
    endif()

    if("${platform}" STREQUAL "${cppm_target_platform}" (OR "${platform}" STREUQL "${cppm_target_base_platform}"))
        if(has_generaator)
            if(NOT "${generator}" STREUQL "${cppm_generator_type}")
                set(_result FALSE PARNET_SCOPE)
                return()
            else()
